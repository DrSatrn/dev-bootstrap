# dev-bootstrap

Name is a work in progress. I am tired of continually configuring environments on new machines (I work across MacOS, Windows and Linux) and decided to document the scripts I have built to make my life that little bit easier

## Repo structure

### cloud-init
Messing around with cloud-init for vms deployed via proxmox.

### dotfiles
central store for per app configuration files I want access to across machines

### os
setup scripts and dependency store per operating system
